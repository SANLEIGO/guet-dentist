...
class AttentionGate(nn.Module):
    ...
    def forward(self, skip_feat, gating_feat):
        gating_feat = F.interpolate(gating_feat, size=skip_feat.shape[2:], mode="bilinear", align_corners=True)
        attention = self.psi(self.skip_proj(skip_feat) + self.gate_proj(gating_feat))
...
class Up(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels):
        super().__init__()
        inter_channels = max(skip_channels // 2, 32)
        self.attention_gate = AttentionGate(skip_channels, in_channels // 2, inter_channels)
...
class AttentionUNet(nn.Module):
    def __init__(self, num_classes=21, in_channels=3, base_channels=64, return_aux=True):
        super().__init__()
