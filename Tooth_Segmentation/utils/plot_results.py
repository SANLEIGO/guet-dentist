...
def plot_training_curves(train_losses, val_losses, val_metrics_history, weights_folder):
    ...
    mean_iou_list = [m["Tooth IoU"] for m in val_metrics_history]
    dice_list = [m["Tooth Dice"] for m in val_metrics_history]
    boundary_iou_list = [m.get("Boundary IoU", 0.0) for m in val_metrics_history]
    ...
    plt.plot(epochs, mean_iou_list, label="Tooth IoU", linewidth=2)
    plt.plot(epochs, dice_list, label="Tooth Dice", linewidth=2)
    plt.plot(epochs, boundary_iou_list, label="Boundary IoU", linewidth=2)
