...
def segment_teeth_with_instances(
    image: np.ndarray,
    method: str = "alphadent",
    ...
    if method == "alphadent":
        ...
        if model is None:
            global _SEG_MODEL_ERROR
            error_msg = _SEG_MODEL_ERROR if _SEG_MODEL_ERROR else "model_unavailable"
        ...
        # U-Net不支持实例分割
        return _empty_instance_result(image, "unet_does_not_support_instances")


def fallback_full_mask(image: np.ndarray) -> SegmentationResult:
    """Create a fallback full-image mask when segmentation fails.

    This is used when segmentation fails completely and we need a mask
    ...
    Returns:
        SegmentationResult with full-image mask
    """
    h, w = image.shape[:2]
