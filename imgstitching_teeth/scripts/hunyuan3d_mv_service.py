    ...
    def _ensure_pipeline_loaded(self):
        ...
        with self._pipeline_lock:
            ...
            kwargs = {
                ...
                "device": self.device,
                "use_safetensors": self.subfolder != "hunyuan3d-dit-v2-1",
            }
            if self.device.startswith("cuda") and kwargs["use_safetensors"]:
                kwargs["variant"] = "fp16"
...
async def health():
    ...
    return {
        "status": "ok",
        "message": "Hunyuan3D single-image bridge is ready.",
        "jobs": len(_jobs),
        "queued_jobs": queued,
...
def _format_runtime_error(exc):
    ...
    if "incomplete metadata" in text or "file not fully covered" in text:
        return "本地 Hunyuan3D 模型文件不完整或已损坏，请在启动器里重新执行“仅下载模型”后重试。"
    return text
...
def main():
    parser = argparse.ArgumentParser(description="Local Hunyuan3D-2.1 single-image bridge service.")
